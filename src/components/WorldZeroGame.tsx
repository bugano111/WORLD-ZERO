...
export function WorldZeroGame(){
 ...
 useEffect(()=>{
  ...
  const insects:THREE.Mesh[]=[];for(let i=0;i<36;i++){const im=new THREE.Mesh(new THREE.SphereGeometry(.035,5,4),insectMat);scene.add(im);insects.push(im)}
  ...
  let yaw=0,last=performance.now();const clock=(now:number)=>{const dt=Math.min(.04,(now-last)/1000);last=now;const j=input.current;
   ...
   for(let i=0;i<insects.length;i++){const a=now*.0012+i*.73,r=3+(i%8)*.7;const x=human.position.x+Math.cos(a)*r,z=human.position.z+Math.sin(a*1.17)*r;insects[i].position.set(x,H(x,z)+.5+(i%5)*.16+Math.sin(a*3)*.18,z)}
