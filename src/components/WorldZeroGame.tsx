...
export function WorldZeroGame(){
 ...
 useEffect(()=>{
  ...
  let renderer:THREE.WebGLRenderer;try{renderer=new THREE.WebGLRenderer({antialias:false,powerPreference:"default"});}catch(err){console.error(err);setStatus("R80 · WEBGL NELZE SPUSTIT");setReady(true);return;}
  ...
  },undefined,e=>{console.error("R61 human load failed",e);setStatus("R80 · CHYBA MODELU POSTAVY");setReady(true)});
  ...
  let raf=0;const loop=(now:number)=>{const dt=Math.min(.025,(now-last)/1000);last=now;const j=input.current;yaw+=j.look*dt*1.8;const f=new THREE.Vector3(Math.sin(yaw),0,Math.cos(yaw)),r=new THREE.Vector3(f.z,0,-f.x);
   ...
   try{renderer.render(scene,camera)}catch(e){console.error("R80 render",e);setStatus("R80 · CHYBA RENDERU");setReady(true);return}raf=requestAnimationFrame(loop)};
