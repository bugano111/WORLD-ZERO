...
export function WorldZeroGame(){
 ...
 const [status,setStatus]=useState("REALISM 68 · REAL FOREST…"),[wood,setWood]=useState(0),[stone,setStone]=useState(0),[fiber,setFiber]=useState(0);
 useEffect(()=>{
  ...
  for(let i=0;i<520;i++){const a=i*2.399963,r=3+((i*71)%100)/100*145,x=Math.cos(a)*r,z=Math.sin(a)*r;
  ...
  scatterModel(`${import.meta.env.BASE_URL}real-assets/models/pine_sapling_small.gltf`,190,9,220,11.5);
  scatterModel(`${import.meta.env.BASE_URL}real-assets/models/shrub_02.gltf`,260,5,155,.82);
  scatterModel(`${import.meta.env.BASE_URL}real-assets/models/weed_plant_02.gltf`,420,3,115,.55);
  scatterModel(`${import.meta.env.BASE_URL}real-assets/models/tree_stump_01.gltf`,36,10,145,.92);
  scatterModel(`${import.meta.env.BASE_URL}real-assets/models/rock_moss_set_01.gltf`,70,7,155,.32);
  ...
  },undefined,e=>{console.error("R61 human load failed",e);setStatus("R68 · CHYBA MODELU POSTAVY")});
  ...
  const resize=()=>{const w=innerWidth,h=innerHeight;renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix()};resize();addEventListener("resize",resize);setStatus("REALISM 68 · DEN 1 · REAL FOREST");requestAnimationFrame(loop);
