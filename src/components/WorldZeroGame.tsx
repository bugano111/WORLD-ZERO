...
function World3D({input,onCounts,onTarget,gatherApi}:{input:RefObject<InputState>;onCounts:(v:{branches:number;sticks:number;stones:number;leaves:number})=>void;onTarget:(v<Kind|null)=>void;gatherApi:RefObject<()=>void>}){
 ...
 useEffect(()=>{
  ...
  const groundMat=new THREE.MeshStandardMaterial({color:0x63894b,roughness:1,metalness:0});
  ...
  for(let i=0;i<pos.count;i++){const x=pos.getX(i),y=pos.getY(i);pos.setZ(i,.20*Math.sin(x*.17)+.13*Math.cos(y*.23)+.06*Math.sin((x+y)*.41));}
  ground.geometry.computeVertexNormals();scene.add(ground);

  const mountainMat=new THREE.MeshStandardMaterial({color:0x71886a,roughness:1});
  ...
  const head=new THREE.Mesh(new THREE.SphereGeometry(.25,16,12),skin);head.position.y=1.72;head.castShadow=true;player.add(head);
  ...
  treePts.forEach(([x,z],i)=>{
   ...
   const trunk=new THREE.Mesh(new THREE.CylinderGeometry(.48,.72,5.4,12),bark);trunk.position.y=2.7;trunk.castShadow=true;trunk.receiveShadow=true;g.add(trunk);
   const branchGeo=new THREE.CylinderGeometry(.13,.22,2.5,8);
   for(const [rx,rz,ry] of [[-.65,0,4.1],[.65,.25,4.45]] as number[][]){const b=new THREE.Mesh(branchGeo,bark);b.position.set(rx,ry,rz);b.rotation.z=rx<0?-1.0:1.0;b.castShadow=true;g.add(b);}
   const crownY=5.5;
   [[0,0,1.55],[-1.15,.15,1.15],[1.15,.1,1.25],[0,.75,1.2],[0,-.75,1.25],[-.55,0,1.3],[.55,.15,1.3]].forEach((a,j)=>{
    const crown=new THREE.Mesh(new THREE.IcosahedronGeometry(a[2]*(1+(i%3)*.05),2),leafMats[(i+j)%leafMats.length]);crown.position.set(a[0],crownY+(j%2)*.42,a[1]);crown.scale.y=1.08;crown.castShadow=true;crown.receiveShadow=true;g.add(crown);
   });
  ...
  for(let i=0;i<90;i++){const x=((i*37)%101)/101*46-23,z=((i*61)%97)/97*46-23;const blade=new THREE.Mesh(new THREE.ConeGeometry(.045,.34,3),grassMat);blade.position.set(x,.17,z);scene.add(blade);}
  ...
  const chooseTarget=()=>{
   ...
   for(const o of interactives){if(!o.parent)continue;const wp=new THREE.Vector3();o.getWorldPosition(wp);const d=Math.hypot(wp.x-player.position.x,wp.z-player.position.z);
    const kind=o.userData.kind as Kind;if(!kind)continue;const reach=kind==="branch"?1.65:.82;
  ...
  const clock=(now:number)=>{
   ...
   const behind=4.7,high=2.8;camera.position.set(player.position.x-Math.sin(yaw)*behind,high,player.position.z+Math.cos(yaw)*behind);
