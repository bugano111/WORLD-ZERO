...
export function WorldZeroGame(){
 ...
 const [status,setStatus]=useState("REALISM 69 · REAL FOREST…"),[wood,setWood]=useState(0),[stone,setStone]=useState(0),[fiber,setFiber]=useState(0);
 useEffect(()=>{
  ...
  const mossMat=new THREE.MeshStandardMaterial({color:0x405c31,roughness:1});
  const fernMat=new THREE.MeshStandardMaterial({color:0x294b28,roughness:.95,side:THREE.DoubleSide});
  ...
  gltf.load(`${import.meta.env.BASE_URL}real-assets/models/human.glb`,g=>{
    ...
    humanModel.add(shirt,hips,shoeL,shoeR);
  ...
  },undefined,e=>{console.error("R61 human load failed",e);setStatus("R69 · CHYBA MODELU POSTAVY")});
  ...
  const resize=()=>{const w=innerWidth,h=innerHeight;renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix()};resize();addEventListener("resize",resize);setStatus("REALISM 69 · DEN 1 · REAL FOREST");requestAnimationFrame(loop);
