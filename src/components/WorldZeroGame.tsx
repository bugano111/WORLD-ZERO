...
export function WorldZeroGame(){
 ...
 const [status,setStatus]=useState("REALISM 61 · REAL FOREST…"),[wood,setWood]=useState(0),[stone,setStone]=useState(0),[fiber,setFiber]=useState(0);
 useEffect(()=>{
  ...
  loader.load("/WORLD-ZERO/real-assets/models/human.glb",g=>{
  ...
  },undefined,e=>{console.error("R61 human load failed",e);setStatus("R61 · CHYBA MODELU POSTAVY")});
  ...
  const resize=()=>{const w=innerWidth,h=innerHeight;renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix()};resize();addEventListener("resize",resize);setStatus("REALISM 61 · DEN 1 · REAL FOREST");requestAnimationFrame(loop);
