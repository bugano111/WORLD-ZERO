...
export function WorldZeroGame(){
 ...
 useEffect(()=>{
  ...
  const H=(x:number,z:number)=>{const r=Math.hypot(x,z),overlook=17*Math.exp(-(x*x)/3600-(z*z)/1900),valley=-46*Math.exp(-((x-18)*(x-18))/23000-((z+165)*(z+165))/13500),n1=Math.sin(x*.018+Math.cos(z*.011))*2.8,n2=Math.sin((x+z)*.041)*1.25,n3=Math.cos(x*.087-z*.063)*.48,cliff=-17*Math.max(0,Math.min(1,(-z-30)/68))*Math.exp(-(x*x)/15000),far=Math.max(0,-z-250)*.075;return 7+overlook+valley+cliff+n1+n2+n3+far+r*.002};
