...
export function WorldZeroGame(){
 ...
 const cloudMat=new THREE.MeshStandardMaterial({color:0xf3f0e8,transparent:true,opacity:.17,roughness:1,depthWrite:false});for(let i=0;i<8;i++){const cloud=new THREE.Group();for(let k=0;k<9;k++){const puff=new THREE.Mesh(new THREE.IcosahedronGeometry(8+(k%4)*3,2),cloudMat);puff.scale.set(1.9+(k%3)*.25,.55+(k%2)*.18,1);puff.position.set((k-4)*9+(k%2)*4,Math.sin(k*1.7)*4,(k%3)*-4);cloud.add(puff)}cloud.position.set(-250+i*75,82+(i%3)*14,-315-(i%4)*38);scene.add(cloud)}
  ...
  let humanModel:THREE.Object3D|null=null,humanMixer:THREE.AnimationMixer|null=null;let gaitPhase=0;
  ...
  let raf=0;const loop=(now:number)=>{const dt=Math.min(.025,(now-last)/1000);last=now;const j=input.current;yaw+=j.look*dt*1.8;const f=new THREE.Vector3(Math.sin(yaw),0,Math.cos(yaw)),r=new THREE.Vector3(f.z,0,-f.x);
   const speed=Math.min(1,Math.hypot(j.x,j.y));human.position.addScaledVector(f,-j.y*dt*6);human.position.addScaledVector(r,-j.x*dt*6);human.position.y=H(human.position.x,human.position.z)+.008;const moving=speed>.05;setHumanAction(moving?walkAction:idleAction);if(walkAction)walkAction.timeScale=.78+speed*.55;humanMixer?.update(dt);gaitPhase+=dt*(moving?7.2*speed:1.35);if(humanModel){const breathe=Math.sin(gaitPhase*(moving?.45:1))*(moving?.004:.009);humanModel.position.y=breathe+(moving?Math.abs(Math.sin(gaitPhase))*.012:0);humanModel.rotation.z=THREE.MathUtils.lerp(humanModel.rotation.z,moving?Math.sin(gaitPhase)*.012:0,1-Math.exp(-dt*8));humanModel.rotation.x=THREE.MathUtils.lerp(humanModel.rotation.x,moving?.018:0,1-Math.exp(-dt*6));}human.rotation.y=yaw;if(Math.hypot(j.x,j.y)>.05)human.rotation.y=Math.atan2(-j.x,-j.y)+yaw;
