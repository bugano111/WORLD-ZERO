...
export function WorldZeroGame(){
 ...
 useEffect(()=>{
  ...
  let renderer:THREE.WebGLRenderer;try{renderer=new THREE.WebGLRenderer({antialias:false,powerPreference:"default"});}catch(err){console.error(err);setStatus("R90 · WEBGL NELZE SPUSTIT");setReady(true);return;}
  ...
  const H=(x:number,z:number)=>Math.sin(x*.018)*5.2+Math.cos(z*.021)*4.4+Math.sin((x+z)*.008)*7.5+Math.sin(Math.hypot(x,z)*.014)*3.5;
  ...
  gltf.load(`${import.meta.env.BASE_URL}real-assets/models/human.glb`,g=>{
    ...
    setReady(true); setStatus("REALISM 90 · DEN 1 · REAL FOREST");
  ...
  },undefined,e=>{console.error("R61 human load failed",e);setStatus("R90 · CHYBA MODELU POSTAVY");setReady(true)});
  // Never leave iPhone behind the loading curtain if a slow/broken asset stalls.
  const bootGuard=window.setTimeout(()=>{setReady(true);setStatus("REALISM 90 · SVĚT SPUŠTĚN")},6500);
  ...
  let raf=0;const loop=(now:number)=>{const dt=Math.min(.025,(now-last)/1000);last=now;const j=input.current;yaw+=j.look*dt*1.8;const f=new THREE.Vector3(Math.sin(yaw),0,Math.cos(yaw)),r=new THREE.Vector3(f.z,0,-f.x);
   ...
   try{renderer.render(scene,camera)}catch(e){console.error("R90 render",e);setStatus("R90 · CHYBA RENDERU");setReady(true);return}raf=requestAnimationFrame(loop)};
