...
export function WorldZeroGame(){
 ...
 const [status,setStatus]=useState("REALISM 64 · REAL FOREST…"),[wood,setWood]=useState(0),[stone,setStone]=useState(0),[fiber,setFiber]=useState(0);
 useEffect(()=>{
  ...
  gltf.load(`${import.meta.env.BASE_URL}real-assets/models/human.glb`,g=>{
    ...
    const scale=1.78/Math.max(.01,size.y); // canonical adult height: 1.78 m humanModel.scale.setScalar(scale); humanModel.position.set(-center.x*scale,-box.min.y*scale,-center.z*scale);
  ...
  },undefined,e=>{console.error("R61 human load failed",e);setStatus("R64 · CHYBA MODELU POSTAVY")});
  ...
  const resize=()=>{const w=innerWidth,h=innerHeight;renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix()};resize();addEventListener("resize",resize);setStatus("REALISM 64 · DEN 1 · REAL FOREST");requestAnimationFrame(loop);
