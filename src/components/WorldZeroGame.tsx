...
export function WorldZeroGame(){
 ...
 const [status,setStatus]=useState("REALISM 71 · REAL FOREST…"),[wood,setWood]=useState(0),[stone,setStone]=useState(0),[fiber,setFiber]=useState(0);
 useEffect(()=>{
  if(!host.current)return;
  const el=host.current,scene=new THREE.Scene();
  scene.background=new THREE.Color(0x9fb7bd);scene.fog=new THREE.FogExp2(0x75877b,.0072);
  ...
  const sun=new THREE.DirectionalLight(0xffe2b7,3.2);sun.position.set(-45,70,-25);sun.castShadow=true;sun.shadow.mapSize.set(1024,1024);sun.shadow.camera.left=-80;sun.shadow.camera.right=80;sun.shadow.camera.top=80;sun.shadow.camera.bottom=-80;scene.add(sun);
  ...
  for(let i=0;i<240;i++){const a=i*2.399963,r=3+((i*71)%100)/100*145,x=Math.cos(a)*r,z=Math.sin(a)*r;
  ...
  new RGBELoader().load(`${import.meta.env.BASE_URL}real-assets/mossy_forest_panorama.hdr`,hdr=>{hdr.mapping=THREE.EquirectangularReflectionMapping;scene.environment=hdr;scene.background=hdr;scene.backgroundBlurriness=.32;});
  ...
  scatterModel(`${import.meta.env.BASE_URL}real-assets/models/pine_sapling_small.gltf`,110,10,220,9.2);
  scatterModel(`${import.meta.env.BASE_URL}real-assets/models/shrub_02.gltf`,150,6,155,.78);
  scatterModel(`${import.meta.env.BASE_URL}real-assets/models/weed_plant_02.gltf`,210,4,115,.52);
  scatterModel(`${import.meta.env.BASE_URL}real-assets/models/tree_stump_01.gltf`,24,10,145,.88);
  scatterModel(`${import.meta.env.BASE_URL}real-assets/models/rock_moss_set_01.gltf`,42,8,155,.3);
  ...
  gltf.load(`${import.meta.env.BASE_URL}real-assets/models/human.glb`,g=>{
    ...
    // R71 fail-safe: the source body can render without a visible head on some devices.
    // Add a correctly scaled anatomical head/neck so the playable human is never headless.
    const skinHead=new THREE.MeshStandardMaterial({color:0xc98f6b,roughness:.72,metalness:0});
    const hairMat=new THREE.MeshStandardMaterial({color:0x2b211c,roughness:.9});
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.065,.075,.12,20),skinHead); neck.position.set(0,1.54,0); neck.castShadow=true;
    const head=new THREE.Mesh(new THREE.SphereGeometry(.115,28,22),skinHead); head.scale.set(.88,1.18,.92); head.position.set(0,1.69,0); head.castShadow=true;
    const hair=new THREE.Mesh(new THREE.SphereGeometry(.119,28,16,0,Math.PI*2,0,Math.PI*.48),hairMat); hair.scale.set(.89,1.12,.93); hair.position.set(0,1.715,0); hair.castShadow=true;
    human.add(neck,head,hair);
  ...
  },undefined,e=>{console.error("R61 human load failed",e);setStatus("R71 · CHYBA MODELU POSTAVY")});
  ...
  const loop=(now:number)=>{const dt=Math.min(.025,(now-last)/1000);last=now;const j=input.current;yaw+=j.look*dt*1.8;const f=new THREE.Vector3(Math.sin(yaw),0,Math.cos(yaw)),r=new THREE.Vector3(f.z,0,-f.x);
  ...
  const resize=()=>{const w=innerWidth,h=innerHeight;renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix()};resize();addEventListener("resize",resize);setStatus("REALISM 71 · DEN 1 · REAL FOREST");requestAnimationFrame(loop);
