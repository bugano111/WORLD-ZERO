...
export function WorldZeroGame(){
 ...
 useEffect(()=>{
  ...
  const sun=new THREE.DirectionalLight(0xffc27d,2.25);sun.position.set(-145,52,-70);sun.castShadow=true;sun.shadow.mapSize.set(mobile?384:1024,mobile?384:1024);sun.shadow.camera.left=-80;sun.shadow.camera.right=80;sun.shadow.camera.top=80;sun.shadow.camera.bottom=-80;scene.add(sun);
  ...
  const g=new THREE.PlaneGeometry(700,700,mobile?64:112,mobile?64:112);g.rotateX(-Math.PI/2);const pa=g.attributes.position as THREE.BufferAttribute;
 ...
 const cloudMat=new THREE.MeshStandardMaterial({color:0xf3f0e8,transparent:true,opacity:.17,roughness:1,depthWrite:false});for(let i=0;i<8;i++){const cloud=new THREE.Group();for(let k=0;k<9;k++){const puff=new THREE.Mesh(new THREE.IcosahedronGeometry(8+(k%4)*3,2),cloudMat);puff.scale.set(1.9+(k%3)*.25,.55+(k%2)*.18,1);puff.position.set((k-4)*9+(k%2)*4,Math.sin(k*1.7)*4,(k%3)*-4);cloud.add(puff)}cloud.position.set(-250+i*75,82+(i%3)*14,-315-(i%4)*38);scene.add(cloud)}
  ...
  const trunkMat=new THREE.MeshStandardMaterial({color:0x39291f,roughness:1});const needleMats=[0x183824,0x21452c,0x2b5233].map(color=>new THREE.MeshStandardMaterial({color,roughness:.98}));
  const trunkGeo=new THREE.CylinderGeometry(.14,.28,7.2,7),crownGeos=[new THREE.ConeGeometry(2.2,4.2,9),new THREE.ConeGeometry(1.75,3.8,9),new THREE.ConeGeometry(1.25,3.4,9),new THREE.ConeGeometry(.8,2.8,9)];
  const treeCount=mobile?210:620,trunks=new THREE.InstancedMesh(trunkGeo,trunkMat,treeCount),crowns=crownGeos.map((g,k)=>new THREE.InstancedMesh(g,needleMats[k%3],treeCount));const dummy=new THREE.Object3D(),groundY=(x:number,z:number)=>H(x,z);
  for(let i=0;i<treeCount;i++){const a=i*2.399963+(i%11)*.047,r=23+((i*53)%101)/100*340;let x=Math.cos(a)*r,z=Math.sin(a)*r;if(z<-72&&Math.abs(x)<54)x+=(x<0?-1:1)*(62+(i%8)*4);const h=.82+(i%13)*.035,rot=(i*.73)%(Math.PI*2);dummy.position.set(x,groundY(x,z)+3.58*h-.12,z);dummy.rotation.set(0,rot,0);dummy.scale.set(h,h,h);dummy.updateMatrix();trunks.setMatrixAt(i,dummy.matrix);for(let k=0;k<4;k++){dummy.position.set(x,groundY(x,z)+(4.62+k*1.65)*h-.12,z);dummy.rotation.set(0,rot+k*.31,0);dummy.scale.set(h*(1-k*.08),h,h*(1-k*.08));dummy.updateMatrix();crowns[k].setMatrixAt(i,dummy.matrix)}}trunks.castShadow=true;trunks.receiveShadow=true;scene.add(trunks);crowns.forEach(m=>{m.castShadow=!mobile;m.receiveShadow=true;scene.add(m)});
